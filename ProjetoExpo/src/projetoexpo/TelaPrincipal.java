...
public class TelaPrincipal extends JFrame {
	...
	private JTextField txtCodigoDeBarra_1;
...
//		timer.start();
		...
		lblDate.setBounds(39, 53, 46, 14);
		contentPane.add(lblDate);

		JLabel lblValorUnitario = new JLabel("Valor Unitario");
		lblValorUnitario.setFont(new Font("Arial", Font.PLAIN, 14));
		lblValorUnitario.setBounds(39, 174, 108, 25);
		...
		txtValorUnitario.setFont(new Font("Arial", Font.PLAIN, 15));
		txtValorUnitario.setBounds(156, 167, 93, 38);
		...
		txtDescricao.setBounds(283, 80, 418, 38);
		...
		lblDescricao.setBounds(283, 42, 222, 25);
		contentPane.add(lblDescricao);

		lblCancelar = new JLabel("Cancelar venda");
		lblCancelar.setFont(new Font("Arial", Font.PLAIN, 14));
		lblCancelar.setBounds(319, 403, 197, 25);
		contentPane.add(lblCancelar);

		lblFinalizar = new JLabel("Finalizar Compra");
		lblFinalizar.setFont(new Font("Arial", Font.PLAIN, 14));
		lblFinalizar.setBounds(39, 403, 191, 25);
		contentPane.add(lblFinalizar);

		lblData = new JLabel(Util.getData());
		lblData.setFont(new Font("Arial", Font.PLAIN, 14));
		lblData.setBounds(94, 48, 93, 25);
		contentPane.add(lblData);

		lblMaisOpcoes = new JLabel("Mais Op\u00E7\u00F5es");
		lblMaisOpcoes.setFont(new Font("Arial", Font.PLAIN, 14));
		lblMaisOpcoes.setBounds(570, 403, 168, 25);
		contentPane.add(lblMaisOpcoes);

		JLabel lblTroco = new JLabel("Troco");
		lblTroco.setFont(new Font("Arial", Font.PLAIN, 14));
		lblTroco.setBounds(583, 304, 118, 25);
		contentPane.add(lblTroco);

		txtTroco = new JTextField();
		txtTroco.setFont(new Font("Arial", Font.PLAIN, 15));
		txtTroco.setBounds(583, 344, 95, 37);
		contentPane.add(txtTroco);
		txtTroco.setColumns(10);
		txtTroco.setEditable(false);

		lblValorRecebido = new JLabel("Valor Recebido");
		lblValorRecebido.setFont(new Font("Arial", Font.PLAIN, 14));
		lblValorRecebido.setBounds(411, 304, 122, 25);
		contentPane.add(lblValorRecebido);

		JButton btnFinalizar = new JButton("");
		btnFinalizar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				txtCodigoDeBarra_1.setText("");
		...
		btnFinalizar.setBounds(39, 441, 214, 83);
		contentPane.add(btnFinalizar);
		
		txtValorRecebido = new JTextField();
		txtValorRecebido.setFont(new Font("Arial", Font.PLAIN, 15));
		txtValorRecebido.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				btnFinalizar.doClick();
			}
		});
		txtValorRecebido.setBounds(411, 334, 138, 56);
		...
		btnCancelar.setBounds(319, 441, 214, 83);
		...
		btnMaisOpcoes.setBounds(572, 441, 214, 83);
		contentPane.add(btnMaisOpcoes);

		scrollPane = new JScrollPane();
		scrollPane.setBounds(283, 150, 469, 146);
		...
		txtCodigoDeBarra_1 = new JTextField();
		txtCodigoDeBarra_1.setFont(new Font("Arial", Font.PLAIN, 15));
		txtCodigoDeBarra_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {

				if (txtCodigoDeBarra_1.getText().equalsIgnoreCase("22222")) {
					btnFinalizar.doClick();
				}

				else if (txtCodigoDeBarra_1.getText().equalsIgnoreCase("33333")) {
					btnCancelar.doClick();
				}

				else if (txtCodigoDeBarra_1.getText().equalsIgnoreCase("44444")) {
					btnMaisOpcoes.doClick();
				}

				else if (txtCodigoDeBarra_1.getText().equalsIgnoreCase("")) {
					JOptionPane.showMessageDialog(null, "Código de Barra vazio");
				}
					...
					try {
						...
						mercadoria.setCodigo(txtCodigoDeBarra_1.getText());
...
//								txtTotal.setText("R$ " + total.toString().format("%.2f", total));
								...
								int confirma = JOptionPane.showConfirmDialog(null, "Deseja Ativá-lo: ", "Confirme", JOptionPane.YES_NO_OPTION);
										
										if (confirma == 0) {
										mercadoria.setAtivo(true);
										mercadoria.atualizarAtivo(conn);
										conn.commit();
										txtCodigoDeBarra_1.setText("");
										} else {
											txtCodigoDeBarra_1.setText("");
							...
							String codigo = txtCodigoDeBarra_1.getText();
		...
		txtCodigoDeBarra_1.setBounds(41, 79, 162, 39);
		contentPane.add(txtCodigoDeBarra_1);
		txtCodigoDeBarra_1.setColumns(10);
	...
	public void cancelaVenda() {
		...
		txtCodigoDeBarra_1.requestFocus();
		txtCodigoDeBarra_1.setText("");
	...
	public void atualizaTotal(int idPedido) {
		pedido.atualizaTotal(conn, idPedido);
